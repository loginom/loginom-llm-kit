...
| 3   | Уникальный идентификатор кэша            | ![](./img/string.svg) Строковый   | LLM. Запрос           |
| 4   | Тайм-аут подключения (мс)                | ![](./img/integer.svg) Целый      | 60000                 |
| 5   | Общий тайм-аут (мс)                      | ![](./img/integer.svg) Целый      | 60000                 |
| 6   | Не проверять сертификат сервера          | ![](./img/logical.svg) Логический | true                  |
| 7   | Задержка между запросами (мс)            | ![](./img/integer.svg) Целый      | 0                     |
| 8   | Количество повторных запросов при ошибке | ![](./img/integer.svg) Целый      | 0                     |
| 9   | Задержка между неудачными запросами (мс) | ![](./img/integer.svg) Целый      | 1000                  |
...
3. **Уникальный идентификатор кэша** — имя, под которым будет сохраняться результат работы текущего узла.
...
5. **Общий тайм-аут (мс)** — максимальное время, которое будет затрачено на отправку HTTP-запроса и получение ответа, по истечении которого обмен будет прерван со статусом ошибки. Если параметр равен нулю, то таймаут не ограничен.
6. **Не проверять сертификат сервера** — если данный флаг установлен, то ошибки при проверке сертификата сервера, к которому производится подключение, игнорируются.
7. **Задержка между запросами (мс)** — время ожидания перед каждым отдельным запросом из списка запросов.
8. **Количество повторных запросов при ошибке** — количество повторных попыток выполнения запроса при возникновении ошибки. По умолчанию `0`.
9. **Задержка между неудачными запросами (мс)** — время ожидания перед повторной попыткой выполнения запроса при возникновении ошибки.
