...
| 6   | Уникальный идентификатор кэша            | ![](./img/string.svg) Строковый   | OpenRouter            |
| 7   | Тайм-аут подключения (мс)                | ![](./img/integer.svg) Целый      | 60000                 |
| 8   | Общий тайм-аут (мс)                      | ![](./img/integer.svg) Целый      | 60000                 |
| 9   | Не проверять сертификат сервера          | ![](./img/logical.svg) Логический | true                  |
| 10  | Задержка между запросами (мс)            | ![](./img/integer.svg) Целый      | 0                     |
| 11  | Количество повторных запросов при ошибке | ![](./img/integer.svg) Целый      | 0                     |
| 12  | Задержка между неудачными запросами (мс) | ![](./img/integer.svg) Целый      | 1000                  |
...
6. **Уникальный идентификатор кэша** — имя, под которым будет сохраняться результат работы текущего узла.
...
8. **Общий тайм-аут (мс)** — максимальное время, которое будет затрачено на отправку HTTP-запроса и получение ответа, по истечении которого обмен будет прерван со статусом ошибки. Если параметр равен нулю, то таймаут не ограничен.
9. **Не проверять сертификат сервера** — если данный флаг установлен, то ошибки при проверке сертификата сервера, к которому производится подключение, игнорируются.
10. **Задержка между запросами (мс)** — время ожидания перед каждым отдельным запросом из списка запросов.
11. **Количество повторных запросов при ошибке** — количество повторных попыток выполнения запроса при возникновении ошибки. По умолчанию `0`.
12. **Задержка между неудачными запросами (мс)** — время ожидания перед повторной попыткой выполнения запроса при возникновении ошибки.
